...
const CameraComponent = ({ navigation, close, closer, uriCallback }) => {
  ...
  const takePicture = async function (camera) {
    const options = { quality: 0.5, base64: true, mirrorImage: !cameraView };
    const data = await camera.takePictureAsync(options);
    if (uriCallback) {
      uriCallback(data.uri);
      closer();
    } else {
      setPreview(data.uri);
    }
    ...
    >
      ...
      >
        {({ camera, status, recordAudioPermissionStatus }) => {
          if (status !== "READY") return <Loader transparent={true} />;
          return (
            <View
              style={{
                flexDirection: "row",
            ...
            >
              ...
              {close && (
                <TouchableOpacity onPress={closer} style={styles.close}>
